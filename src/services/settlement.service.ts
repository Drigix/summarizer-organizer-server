...
import {
  ...
  VerticalBarModel,
} from 'src/models/vertical-bar.model';
...
export class SettlementService {
  ...
  async saveFromSavingSettlement(
    settlementSavingDto: SettlementSavingDto,
  ): Promise<Settlement> {
    const settlement =
      this.convertSavingSettlementToSoldInvestment(settlementSavingDto);
    if (settlement == null) {
      return null;
    }
  ...
  async deleteById(id: string): Promise<void> {
    await this.settlementModel.findByIdAndDelete(id).exec();
  }

  private convertSavingSettlementToSoldInvestment(
    settlementSavingDto: SettlementSavingDto,
  ): Settlement {
    const priceType = PriceUtils.convertPriceToSettlementPriceType(
      settlementSavingDto.price,
    ...
    settlement.date = new Date(
      DateUtil.getFirstDayOfMonth(settlementSavingDto.sellDate),
    );
    settlement.description = settlementSavingDto.description;
    settlement.linkUrl = settlementSavingDto.linkUrl;
    settlement.price = PriceUtils.calculateSellPrice(
      settlementSavingDto.price,
      settlementSavingDto.currentPrice,
    );
