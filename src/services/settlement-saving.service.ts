...
export class SettlementSavingService {
  ...
  constructor(
    ...
    private refreshPriceService: RefreshPriceService
  ) {}

  async save(
    settlementSavingDto: SettlementSavingDto,
  ): Promise<SettlementSaving> {
    const settlementSaving = new this.settlementSavingModel(
      settlementSavingDto,
    );
  ...
  async update(
    ...
    return this.settlementSavingModel.findByIdAndUpdate(
      id,
      settlementSavingDto,
    );
  }
  ...
  async refreshGoldAndSilverPrices(ids: string[]): Promise<SettlementSaving[]> {
    ...
    const resultList = await Promise.all(
      ids.map(async (id) => {
        let settlementSaving = await this.settlementSavingModel.findById(id);

        if (settlementSaving && settlementSaving.refreshPriceUrl) {
          const currentPrice =
            await this.refreshPriceService.getCurrentGoldOrSilverPrice(
              settlementSaving.refreshPriceUrl,
            );

          settlementSaving = await this.settlementSavingModel.findByIdAndUpdate(
            id,
      ...
      .aggregate([
        ...
        },
        {
          $group: {
            ...
            price: {
              $sum: {
                $multiply: ['$price', { $ifNull: ['$amount', 1] }],
              },
            },
          },
        },
  ...
  async findBondsAndDepositsWithProfit(
    ...
    const deposits = settlements.filter(
    ...
    bonds.forEach((b) => {
      ...
      for (let i = monthsDiff; i >= 0; i--) {
        ...
        if (index > 1) {
          const newProfit =
            b.price + (b.price * index * (b.percent / 100)) / b.percentPeriod;
          if (searchElement) {
